use gir_parser::{prelude::*, TransferOwnership};
use std::{
...
impl Parameter {
    ...
    pub fn c_type(&self) -> &str {
        match self {
            Parameter::Instance {
                ...
                c_type_override,
                ..
            } => c_type_override.as_deref().or(param.ty().and_then(|t| t.c_type())),
            Parameter::Return {
                param,
                c_type_override,
                ..
    ...
    pub fn name(&self) -> &str {
        match self {
            Self::Instance { param, name_override, .. } => name_override.as_deref().or(Some(param.name())),
            Self::Default { param, name_override, .. } => name_override.as_deref().or(Some(param.name())),
    ...
    pub fn set_tid(&mut self, new_tid: TypeId) {
        match self {
            Self::Instance { ref mut tid, .. }
            | Self::Return { ref mut tid, .. }
            | Self::Default { ref mut tid, .. } => {
                *tid = new_tid;
            }
            Self::Error(ref mut tid) | Self::VarArgs(ref mut tid) | Self::None(ref mut tid) => {
                *tid = new_tid;
            }
        }
    }
