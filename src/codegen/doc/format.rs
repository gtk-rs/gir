...
fn replace_symbols(
    ...
    if env.config.use_gi_docgen || env.library.doc_format == DocFormat::GiDocgen {
...
fn find_method_or_function_by_ctype(
    ...
    find_method_or_function(
        ...
        in_type,
        |f| f.glib_name == name,
        |o| c_type.is_none_or(|t| o.c_type == t),
        |r| c_type.is_none_or(|t| r.type_(&env.library).c_type == t),
        |r| c_type.is_none_or(|t| r.type_(&env.library).c_type == t),
        |r| c_type.is_none_or(|t| r.type_(&env.library).c_type == t),
